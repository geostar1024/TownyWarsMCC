...
class WarExecutor implements CommandExecutor
{
  ...
  private boolean declareWar(CommandSender cs, String[] strings, boolean admin)
  {
    ...
    try
    {
      ...
      {
        declarer = TownyWars.nationToTownyWarsNationHash.get(TownyUniverse.getDataSource().getNation(strings[1]));
        target = TownyWars.nationToTownyWarsNationHash.get(TownyUniverse.getDataSource().getNation(strings[2]));
      }
      else
      {
        resident = TownyUniverse.getDataSource().getResident(cs.getName());
        declarer = TownyWars.nationToTownyWarsNationHash.get(resident.getTown().getNation());
        target = TownyWars.nationToTownyWarsNationHash.get(TownyUniverse.getDataSource().getNation(strings[1]));
      }
    }
    catch (Exception ex)
    {
    	if (admin) {
    		cs.sendMessage(ChatColor.RED+"You specified one or more invalid nations!");
    	}
    	else {
    		cs.sendMessage(ChatColor.RED + "You are not in a town, or your town isn't part of a nation, or you specified an invalid nation!");
    	}
      return false;
    }
    if ((!admin) && (!declarer.getNation().isKing(resident)) && (!declarer.getNation().hasAssistant(resident)))
    ...
    WarManager.quickWar(declarer, target);
